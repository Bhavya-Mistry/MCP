...
FILES_DIR = BASE_DIR.parent / "data" / "files"
CHROMA_DIR = BASE_DIR.parent / "chroma_db"
FILES_DIR.mkdir(parents=True, exist_ok=True)
CHROMA_DIR.mkdir(parents=True, exist_ok=True)

# 2. Global variables (initialized inside register_tools to avoid early timeouts)
model = None
collection = None
...
def register_tools(mcp):
    ...
    def index_file(file_name: str) -> str:
        ...
        if not os.path.exists(path):
            return f"File {file_name} not found in {FILES_DIR}"
        
        # ... (rest of your existing index_file logic) ...
        # Ensure you use the global 'collection' and 'model' variables
        return f"Successfully indexed {file_name}"
